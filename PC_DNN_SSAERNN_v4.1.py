...
MF, Denoise(Wavelet), AGC에 따른 결과 차이 확인하기 위해 코드 전반적으로 수정
    ...
    # v4.1
    - cost epoch에 따라 저장하도록 수정
    - ver2, ver3도 input받아 동작하도록
...
# Data load
filepath_data = 'D://7. Research/1. People Counting/2. People Counting based on DNN/0. Data/Dataset_Atlas_ver3.mat' # Signal data path

# ver2, 3의 경우
"""
ver2: signal별 normalize
ver3: min, max 전체 dataset에서 계산 후 normalize

f_i_ver2
0: AGC X, Denon X
1: ACG X, Wavelet 4 Denoise
2: ACG X, Threshold based Denoise
3: ACG O, Denon X
4: ACG O, Wavelet 4 Denoise
5: ACG O, Threshold based Denoise
f_j_ver2
0: correlation X
1: correlation O
"""
...
#list_drop = [droprate1]
#list_noiselevel = [d1_noiselevel[3]]
# =============================================================================
...
d2_avg_cost_training = np.zeros([EPOCH,len(models)])
d2_avg_cost_test = np.zeros([EPOCH,len(models)])
...
#plt.plot(np.arange(1,EPOCH+1),d2_avg_cost_test)
#plt.plot(np.arange(1,EPOCH+1),d2_avg_cost_training)
#plt.show()
...
#save_path = saver.save(sess,os.path.join(MODEL_PATH, 'trained_Atlas_v2.ckpt'))  
