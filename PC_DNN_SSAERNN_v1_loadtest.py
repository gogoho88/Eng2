...
#            self.LSTM_X = tf.Variable(self.Node_hidden3_stack, dtype=tf.float32, name='LSTM_X')
            ...
            self.output_fw = tf.transpose(self.Node_output[0],[1,0,2])
            self.output_bw = tf.transpose(self.Node_output[1],[1,0,2])
            
            self.output_concat = tf.concat([self.output_fw[-1], self.output_bw[-1]], axis=1)
            
            self.logit = tf.matmul(self.output_concat,self.W) + self.B
