...
def data_Y_tile(data_Y, num):
    Y_size = np.shape(data_Y)
    
    data_output = np.zeros([Y_size[0], num, Y_size[1]])
    for m in range(Y_size[0]):
        d1_temp = data_Y[m,:]
        d2_temp = np.tile(d1_temp,[num,1])
        
        data_output[m,:,:] = d2_temp
...
for n in range(num_people+1):
    ...
    for m in range(size_dataset_temp):
        ...
        d2_frame_resize = np.reshape(d2_frame,[frame_len, size_y, 1])
...
#            self.B3 = tf.get_default_graph().get_collection('B3')[m_idx]
            ...
            self.Y = tf.placeholder(tf.int32, [None, 2*n_input_steps, n_class])
...
#                Node_hidden3 = tf.reshape(Node_hidden3, [-1,1,n_AE_hidden3])
            self.LSTM_fw_cell = tf.nn.rnn_cell.LSTMCell(num_units=n_hidden, dtype = tf.float32)
            self.LSTM_fw_cell = tf.nn.rnn_cell.DropoutWrapper(self.LSTM_fw_cell, output_keep_prob = self.keep_prob)
            self.LSTM_fw_cell = tf.contrib.rnn.OutputProjectionWrapper(self.LSTM_fw_cell, output_size = n_class)
            
            self.LSTM_bw_cell = tf.nn.rnn_cell.LSTMCell(num_units=n_hidden, dtype = tf.float32)
            self.LSTM_bw_cell = tf.nn.rnn_cell.DropoutWrapper(self.LSTM_bw_cell, output_keep_prob = self.keep_prob)
            self.LSTM_bw_cell = tf.contrib.rnn.OutputProjectionWrapper(self.LSTM_bw_cell, output_size = n_class)
            
            self.Node_output,_ = tf.nn.bidirectional_dynamic_rnn(self.LSTM_fw_cell, self.LSTM_bw_cell, self.X, dtype = tf.float32)
...
#        with tf.variable_scope(self.name):
            self.Y_avg = tf.reduce_mean(self.Y, 1)
            self.output_avg = tf.reduce_mean(self.Node_output_concat, 1)
            
            self.Loss = tf.losses.mean_squared_error(labels=self.Y, predictions=self.Node_output_concat)
            self.optimizer = tf.train.AdamOptimizer(Learning_rate).minimize(self.Loss)
            self.correct_prediction = tf.equal(tf.argmax(self.output_avg, 1), tf.argmax(self.Y_avg, 1))
...
#        batch_xs, batch_ys = get_minibatch(X_train, y_train, MINIBATCH_SIZE)
    ...
    batch_ys = data_Y_tile(batch_ys, frame_len*2)
